...
BEGIN
    ...
    -- Insert into local node if this record belongs here (>= 2022 AND < 2024)
    IF new_startYear IS NOT NULL AND new_startYear >= 2022 AND new_startYear < 2024 THEN
...
BEGIN
    ...
    IF (old_startYear >= 2024) AND (new_startYear >= 2022 AND new_startYear < 2024) THEN
        -- Moving from Node A to Node B - insert locally
        INSERT INTO title_ft
        VALUES (new_tconst, new_primaryTitle, new_runtimeMinutes,
                new_averageRating, new_numVotes, updated_weightedRating, new_startYear);
    ELSEIF (old_startYear >= 2022 AND old_startYear < 2024) AND (new_startYear IS NULL OR new_startYear < 2022 OR new_startYear >= 2024) THEN
        -- Moving from Node B to Node A or out of range - delete locally
        DELETE FROM title_ft WHERE tconst = new_tconst;
    ELSEIF new_startYear >= 2022 AND new_startYear < 2024 THEN
