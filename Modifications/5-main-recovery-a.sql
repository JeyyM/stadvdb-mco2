-- ============================================================================
-- RECOVERY FOR NODE A (Run on MAIN)
-- Main reads its own transaction_log and pushes missing transactions to Node A
-- ============================================================================
...
-- Initialize checkpoint for Node A
INSERT INTO recovery_checkpoint (node_name, last_recovery_timestamp, recovery_count)
VALUES ('node_a', '2000-01-01 00:00:00.000000', 0)
ON DUPLICATE KEY UPDATE node_name = node_name;

SELECT '✅ Recovery checkpoint table ready for Node A' AS status;

-- ============================================================================
-- 1. FIND MISSING TRANSACTIONS ON NODE A
-- ============================================================================
...
BEGIN
    ...
    -- Find committed transactions in Main's log for Node A partition
    -- Query MODIFY rows (which have actual data), not COMMIT rows (which have NULL values)
    SELECT 
        tm.transaction_id,
        tm.timestamp,
        tm.operation_type,
...
-- ============================================================================
-- 2. REPLAY TRANSACTION TO NODE A
-- ============================================================================
...
BEGIN
    ...
    -- Use the provided since_timestamp as the checkpoint (override saved checkpoint)
    SET checkpoint_time = since_timestamp;
...
SELECT '=== Main → Node A Recovery Procedures Created ===' AS status;
SELECT 'Run CALL full_recovery_node_a(NOW() - INTERVAL 24 HOUR) to recover Node A' AS info;
